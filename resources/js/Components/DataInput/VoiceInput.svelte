<script lang="ts">
  ...
  // let secondsBelowThresholdToStop = $state(3);
  ...
  // 		if (currentVolume < volumeThreshold && isRecording && voiceActivationEnabled) {
  // 			let secondsBelowThreshold = 0;
  // 			const intervalId = setInterval(() => {
  // 				if (currentVolume < volumeThreshold) {
  // 					secondsBelowThreshold++;
  // 					if (secondsBelowThreshold >= secondsBelowThresholdToStop) {
  // 						stopRecording();
            ...
            {#if voiceAssistant.isListening && voiceAssistant.currentVolume > 0}
              <div class="flex items-center gap-1">
                ...
                <span class="text-xs opacity-60"
                  >{voiceAssistant.currentVolume}%</span
                >
              </div>
            {/if}
          </div>
